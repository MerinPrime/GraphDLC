...
export class GraphUpdater {
    updateState(graphState: GraphState, tick: number) {
        ...
        for (let i = 0; i < graphState.changedNodes.count; i++) {
            ...
            const isCycleHead = (flags & NodeFlags.CycleHead) !== 0;
            ...
            if (isCycleHead) {
                ...
                markNodeAsChanged(graphState, nodeID);
                continue;
            }
            ...
            if (isChanged) {
                ...
                if (!isDelayed) {
                    if (edgesCount > 0) {
                        ...
                        if (type === BlockerTypeIndex) {
                            ...
                            markNodeAsChanged(graphState, edge);
                        }
                ...
                for (; edgesPointer < lastEdgesPointer; edgesPointer++) {
                    ...
                    markNodeAsChanged(graphState, detector);
                }

                graphState.lastSignals[nodeID] = graphState.signals[nodeID];
            }
            
            if ((isChanged && (flags & NodeFlags.AdditionalUpdate) !== 0) ||
                (tick === 0 && (flags & NodeFlags.EntryPoint) !== 0) ||
        ...
        for (let i = 0; i < graphState.changedNodes.count; i++) {
            ...
            const flags = graphState.flags[nodeID];
            graphState.flags[nodeID] = flags & NodeFlags.NotDirty;
    ...
    resetGraph(graphState: GraphState) {
        ...
        for (let i = 0; i < graphState.flags.length; i++) {
            graphState.flags[i] &= NodeFlags.NotDirty;
        }
        graphState.cycleStates.fill(0);
    }
}

function markNodeAsChanged(graphState: GraphState, nodeID: number) {
    const flags = graphState.flags[nodeID];
    if ((flags & NodeFlags.Dirty) === 0) {
        graphState.flags[nodeID] = flags | NodeFlags.Dirty;
