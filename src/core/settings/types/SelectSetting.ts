...
export class SelectSetting<T extends string | number> extends BaseSetting<T> {
    ...
    constructor(key: string, defaultValue: T, meta: SettingMeta, options: { value: T, label: I18nText }[]) {
    ...
    buildUIComponent(): HTMLSelectElement {
        ...
        select.addEventListener("change", () => {
            const val = typeof this.defaultValue === 'number' ? parseInt(select.value, 10) : select.value;
            this.value = val as T;
        });
        return select;
    }
